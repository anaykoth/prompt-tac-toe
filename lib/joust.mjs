...
import { PASS_MAX_S, TICK } from "../games/joust/src/game/spec.js";
...
/** A trace is the packed 60 Hz input log from games/joust/src/game/spec.js. */
export const MAX_SAMPLES = Math.ceil(PASS_MAX_S / TICK) + 2;

/**
 * The resolver is the same PassSim the browser previews with, so the verdict
 * and the spectacle agree by construction (see games/joust/test/servercheck.mjs).
 */
function simResolve(traceA, traceB, seed) {
  const r = simulatePass({ seed: seed >>> 0, logs: [traceA, traceB] });
  const hits = (r.events ?? [])
    .filter((e) => Number.isFinite(e.points) && e.points !== 0 && (e.seat === 0 || e.seat === 1))
    .map((e) => ({ seat: e.seat, zone: e.type, points: e.points }));
  const unhorsed = r.unseated?.[0] ? 0 : r.unseated?.[1] ? 1 : null;
  return { hits, unhorsed, passScore: r.score, events: r.events, endReason: r.endReason };
}
...
/** One packed tick: [leanX, leanY, aimX, aimY, flags] (see spec.js packInput). */
export function validTrace(t) {
  if (!Array.isArray(t) || t.length === 0 || t.length > MAX_SAMPLES) return false;
  return t.every((s) => Array.isArray(s) && s.length === 5
    && num(s[0], -1, 1) && num(s[1], -1, 1) && num(s[2], -1, 1) && num(s[3], -1, 1)
    && Number.isInteger(s[4]) && s[4] >= 0 && s[4] <= 15);
}
...
export async function submitTrace(seat, passNo, trace) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    if (Array.isArray(result.passScore)) {
      state.score = [state.score[0] + (Number(result.passScore[0]) || 0), state.score[1] + (Number(result.passScore[1]) || 0)];
    } else if (Array.isArray(result.score)) state.score = result.score;
